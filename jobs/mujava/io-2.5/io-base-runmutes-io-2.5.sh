...
echo "out of runmutes process" >> ${output_filename}.txt
echo "`date +"%Y-%m-%d_%H-%M-%S"`" >> ${output_filename}.txt
# Save log files
cd $proj_name/result
for D in `find . -mindepth 1 -maxdepth 1 -type d `
do
echo "doing stuff for ${D}" >> ${output_filename}.txt 
...
echo "done processing results" >> ${output_filename}.txt
# Copy output to home system 
cd "$TMPDIR"/MuJava
cp ${output_filename}.txt $HOME/output/mujava/$proj_name/$exp_name/$result_dir

echo "reached end of batch script" >> ${output_filename}.txt
