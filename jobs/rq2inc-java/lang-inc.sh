...
raw=$(git rev-list 37f8fe8^..HEAD --reverse | head -n 101)
commit_ids=($raw)
...
mvn clean install -DskipTests -Dlicense.skip -gs "$TMPDIR"/settings.xml  >> ${output_filename}.txt
  ...
  mvn org.pitest:pitest-maven:mutationCoverage -DexcludedTestClasses=org.apache.commons.lang3.reflect.FieldUtilsTest -DwithHistory -Dmutators=ALL >> ${output_filename}.txt
