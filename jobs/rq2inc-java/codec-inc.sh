...
raw=$(git rev-list beafa49^..HEAD --reverse | head -n 101)
commit_ids=($raw)
...
mvn clean install -DskipTests -Dlicense.skip -Drat.skip=true -gs "$TMPDIR"/settings.xml  >> ${output_filename}.txt

mvn org.pitest:pitest-maven:mutationCoverage -DexcludedTestClasses=org.apache.commons.codec.digest.MessageDigestAlgorithmsTest -DwithHistory -Dmutators=ALL  -Drat.skip=true >> ${output_filename}.txt
