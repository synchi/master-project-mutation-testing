...
raw=$(git rev-list 4077158^..HEAD --reverse | head -n 101)
commit_ids=($raw)
...
mvn org.pitest:pitest-maven:mutationCoverage -DexcludedTestClasses=org.apache.commons.io.FileUtilsTestCase  -DwithHistory -Dmutators=ALL  >> ${output_filename}.txt
